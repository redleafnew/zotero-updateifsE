startup-begin = Addon is loading
startup-finish = Addon is ready
menuitem-label = Addon Template: Helper Examples
menupopup-label = Addon Template: Menupopup
menuitem-submenulabel = Addon Template
menuitem-filemenulabel = Addon Template: File Menuitem
prefs-title = Green Frog
prefs-table-title = Title
prefs-table-detail = Detail
tabpanel-lib-tab-label = Lib Tab
tabpanel-reader-tab-label = Reader Tab

upifs =
    .label = Update Journal Information from easyScholar
upmeta =
    .label = Update Item Metadata
toolbox =
    .label = Toolbox
cleanExtra =
    .label = Empty "Extra" Field of selected…
cleanExtraAlt=Are you sure you want to empty the content(s) in "Extra" field?
cleanExtraSuc = "Extra" Field emptied.
toolBoxSet = Show Toolbox Menu Settings
auBoldStar =
    .label = Author Bold and/or Asterisk
cleanBold =
    .label = Clean Author Bold
cleanStar =
    .label = Clean Author Asterisk
cleanBoldStar =
    .label = Clean Author Bold and Asterisk
chAuTitle =
    .label = Change Author Name to Title Case
swapAuName =
    .label = Swap Author's First and Last Name
chTitleCase =
    .label = Change Title to Sentense Case
chPubTitle =
    .label = Change Publication Title
chPubTitleCase =
    .label = Change Publication Title Case
itemTitleFindReplace =
    .label = Item Title Find and Replace
showProfile =
    .label = Show Zotero Porfile Directory
showData =
    .label = Show Zotero Data Directory
proDir = The porfile directory is:
dataDir = The data directory is:


# 作者处理
authorProcess = Bold and/or Asterisk for Author Name
authorProcessName = Please input the author name as given name following family name, for example, 'Zhang Wuji':
authorProcessDiaTitle = Author Name Process
boldLabel = Bold
starLabel=Asterisk (*)
boldStarLabel=Bold & Asterisk (*)
cleanBoldLabel=Clean Bold
cleanStarLabel=Clean Asterisk
cleanBoldStarLabel=Clean Bold & Asterisk
zeroItem = No item is selected.
successPubTitleCase=publication(s) was(were) updated successfully. But some words need specific case, like LWT, CyTA. Please check.
authorEmpty = Please input the author name in the above textbox.
authorNoSpace = The author name inputed must be separated by a space.
authorChanged = author(s) updated.
itemAuSwapped = item(s) author(s) first name and last name swapped.
itemAuthorChanged = item(s) author(s) updated.

# 题目条目处理
titleSearchReplaceWin = Item Title Find and Replace
itemTitleFindReplaceLabel = Item Title Find and Replace
titleSearchReplaceLabel = You can input part or whole item title to be replaced, the find and replace case sensitive
titleSearLabel = Find:
titleReplaceLabel = Replace:
titleReplaceButton = Replace
titleEmpty = The find or replace textbox is empty, please input.
findRepSame = The contents you find and replace are same, please check.
itemTitleFindRepSuc = item title(s) replace completed.
allUpcase = The item title is all upcase, please check after the change.
successItemTitleCase = item title(s) case updated.
#
# 期刊题目处理
change-pub-title = Change publication title
change-pub-title-desc = Change publication title, for example change LWT to LWT-Food Science and Technology.
old-pub-title  = Please input the original publication title:
new-pub-title  = Please input the new publication title:
change-title-bn  = Change Publication Title
pubTitleEmpty = The original or the new publication title is empty, please input.
successPubTitle = publication title(s) updated.

# 影响因子
inputSecretkey = Please input easyScholar secretkey in Green Frog preference.
JCR = JCR
IF = IF
IF5 = IF 5 Year
EI = EI
CASUp = CAS Update
CASBasic = CAS Basic
sciUpTop = SCI Up Top
sciUpSmall = SCI Up Small
CSCD = CSCD
PKUCore = PKU Core
CSSCI = CSSCI
SCICore = SCI Tech Core
SSCI = SSCI
AJG = AJG
UTD24 = UTD24
FT50 = FT50
CCF = CCF
FMS = FMS
JCI = JCI
AHCI = AHCI
sciwarn = CAS Warning
esi = ESI
xr = XinRui
xrTop = XinRui Top
xrSmall = XinRui Small
xrWarn = XinRui Warning
compoundIF = Compound IF
comprehensiveIF = Comprehensive IF
njauCore = NJAU Core
njauJour = NJAU High Quality
swufe = SWUFE
cufe = CUFE
uibe = UIBE
sdufe = SDUFE
xdu = XDU
swjtu = SWJTU
ruc = RUC
xmu = XMU
sjtu = SJTU
fdu = FDU
hhu = HHU
scu = SCU
cqu = CQU
nju = NJU
xju = XJU
cug = CUG
cju = CJU
zju = ZJU
cpu = CPU
# 自定义数据集 custom dataset
ami = AMI
clsci = CLSCI
nssf = NSSF
swupl = SWUPL
ABDC = ABDC
Scopus = Scopus
HX = HX
CoreRankings = CORE Rankings
ESCI = ESCI
predatoryReports = Predatory Journal

summary = Summary

upIfsSuccess =
 {$count ->
   [one] Journal information of the selected 1 item updated sucessfully.
  *[other] Journal information of the selected { $count } items updated sucessfully.
  }
upIfsFail = Could not Retrive this journal information from easyScholar.
