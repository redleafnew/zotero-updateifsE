...
export function njauCore(item: Zotero.Item): any {
  ...
  const pubT: any = item.getField("publicationTitle");
  ...
  try {
    var resp = await Zotero.HTTP.request(
      ...
      "http://phq.njau.edu.cn/admin_getJournal2024.action",
      {
        headers: {
          "User-Agent":
            "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36 Edg/114.0.1823.37",
        },
        body: body,
      },
    );
